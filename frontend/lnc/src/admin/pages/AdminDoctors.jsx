          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {doctors.map((doctor) => (
              <tr key={doctor.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  {editingId === doctor.id ? (
                    <input type="text" name="name" value={editForm.name || ''} onChange={handleChange} className="border rounded px-2 py-1 w-full" />
                  ) : (
                    doctor.name
                  )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {editingId === doctor.id ? (
                    <input type="text" name="specialization" value={editForm.specialization || ''} onChange={handleChange} className="border rounded px-2 py-1 w-full" />
                  ) : (
                    doctor.specialization
                  )}
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {editingId === doctor.id ? (
                    <input type="number" name="experience_years" value={editForm.experience_years || ''} onChange={handleChange} className="border rounded px-2 py-1 w-20" />
                  ) : (
                    `${doctor.experience_years} years`
                  )}
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  {editingId === doctor.id ? (
                    <>
                      <button onClick={() => handleSave(doctor.id)} className="text-green-600 hover:text-green-900 mr-3">
                        Save
                      </button>
                    ...
                    <>
                      {!doctor.is_verified && (
                        <button onClick={() => handleVerify(doctor.id)} className="text-indigo-600 hover:text-indigo-900 mr-3">
                          Verify
                        </button>
                      )}
                      <button onClick={() => handleEdit(doctor)} className="text-blue-600 hover:text-blue-900">
                        <PencilIcon className="h-5 w-5" />
                      </button>
                    </>
                  )}
